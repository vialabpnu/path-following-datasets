...
class PathUtils(object):
    ...
    def __init__(self):
        ...
        self.speed_threshold_yaw = 0.2 # m/s
    ...
    def calculate_curvature_using_circle(self,x,y):
        ...
        for i in range(1, len(x) - 1):
            ...
            x3 = x[i + 1]
            y3 = y[i + 1]
            R, curvature_sign = self.find_circle(x1, y1, x2, y2, x3, y3)
            # print("Curvature Sign: {}".format(curvature_sign))
            # print("Curvature Radius: {}".format(R))
            if R is None:
                curvatures.append(0)
            else:
                curvatures.append(1 * curvature_sign / R)
        curvatures.append(curvatures[-1])
        # Floor and Ceiling to two decimal places
        curvatures = [np.floor(curv * 100) / 100 if curv > 0 else np.ceil(curv * 100) / 100 if np.abs(curv) > self.MAX_CURVE else curv for curv in curvatures]
        return curvatures
    ...
    def find_circle(x1, y1, x2, y2, x3, y3):
        ...
        # curvature formula
        # numerator = np.sqrt(((x2 - x1)**2 + (y2 - y1)**2) * ((x3 - x2) ** 2 + (y3 - y2) ** 2) * ((x1 - x3) ** 2 + (y1 - y3) ** 2))
        # denominator = 2 * (np.abs(((x2 - x1) * (y3 - y2)) - ((y2 - y1) * (x3 - x2))))
        ...
        curvature_sign = np.sign(cp)
        
        if R < radius_limit:
            return None, curvature_sign
        return R, curvature_sign 
    
    def calc_yaw_curv(self, x, y, s, s_max, ref_x, ref_y, ref_yaw, current_speed):
        yaw, curv, ds = [], [], []
        # print("Reference X: {}".format(ref_x))
        # print("Reference Y: {}".format(ref_y))
        # print("Reference Distance: {}".format(s))
        current_speed = float(current_speed)
        use_xy_waypoints = True
    ...
    def get_closest_idx_s(self, s, s_ref, total_idx):
        ...
        for i in range(len(closest_indices)):
            # print("Closest Index: {}".format(closest_indices[i]))
    ...
    @staticmethod
    def is_path_increasing_in_distance(path_s):
        for i in range(1, len(path_s)):
            if path_s[i] < path_s[i - 1]:
                return False
        return True