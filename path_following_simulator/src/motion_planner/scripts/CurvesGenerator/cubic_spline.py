...
class Spline:
    ...
    def calc(self, t):
        ...
        # # Quadratic spline
        # result = self.a[i] + self.b[i] * dx + self.c[i] * dx ** 2.0
        # # Linear spline
        # result = self.a[i] + self.b[i] * dx
        return result
    ...
    def calcd(self, t):
        ...
        # # Quadratic spline
        # result = self.b[i] + 2.0 * self.c[i] * dx
        # # Linear spline
        # result = self.b[i]
        return result
    ...
    def lstsq(A, b):
        ...
        Ap = (U * np.sqrt(D)).T
        bp = np.matmul(bA, U) / np.sqrt(D)
        return np.linalg.lstsq(Ap, bp, rcond=None)
    
    # @jit
    # def solve_with_jit(self, A, B):
    #     print("Solving with JIT")
    #     size_row, size_col = A.shape[0], A.shape[1]
    #     empty_array = np.zeros((size_row, 1))
    #     for i in range(size_row):
    #         empty_array[i, :] = solve_impl(A[i], B[i])
    #     return empty_array
